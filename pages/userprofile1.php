...
if(isset($_SESSION['flag'])){
	...
	if(!isset($_GET['rid'])){
        ...
        if(!isset($_GET['pp'])) {
            ...
            $rid = $_GET['rid'];
            require_once 'config.php';
            $con4 = mysql_connect(DB_HOST, DB_USER, DB_PASSWORD);
            if (!$con4) {
                die('Could not connect: ' . mysql_error());
            }
            mysql_select_db(DB_NAME, $con4);
            if($_GET['pp'] == 1){
				//approval a postpone request
                $sql12 = "UPDATE Record SET ExpiredTime = DATE_ADD(ExpiredTime, INTERVAL 3 DAY) WHERE RId = '$rid'";
                mysql_query($sql12, $con4);
                $sql13 = "UPDATE Record SET Postpone = 0 WHERE RId = '$rid'";
                mysql_query($sql13, $con4);
            }
            else{
				//decline a postpone request
                $sql14 = "UPDATE Record SET Postpone = 0 WHERE RId = '$rid'";
                mysql_query($sql14, $con4);
            }
        }
	}
	unset($_SESSION['flag']);
...
require_once 'config.php';
if($_GET['username']==null || $_GET['uid']==null){
	$userprofile_url = "./userprofile.php";
	header('Location:'.$userprofile_url);
}
...
<head>
    ...
    <script type="text/javascript">
        ...
        function postpone(btn){
            ...
            rid = btn.parentNode.parentNode.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling.innerHTML;
